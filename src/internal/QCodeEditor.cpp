...
int QCodeEditor::getFirstVisibleBlock() {
    ...
    for (int i = 0; i < document()->blockCount(); ++i) {
        ...
        QRect r2 = document()
                       ...
                       .translated(viewport()->geometry().x(), viewport()->geometry().y() - verticalScrollBar()->sliderPosition())
                       .toRect();
...
bool QCodeEditor::proceedCompleterBegin(QKeyEvent* e) {
    if (m_completer && m_completer->popup()->isVisible()) {
        switch (e->key()) {
            ...
            case Qt::Key_Escape:
            case Qt::Key_Tab:
            case Qt::Key_Backtab: e->ignore(); return true; // let the completer do default behavior
            default: break;
        }
    }
...
void QCodeEditor::keyPressEvent(QKeyEvent* e) {
    ...
    if (!completerSkip) {
        ...
        // parenthesis
        if (m_autoIndentation && (e->key() == Qt::Key_Return || e->key() == Qt::Key_Enter) && charUnderCursor() == '}' &&
            charUnderCursor(-1) == '{') {
            int charsBack = 0;
            insertPlainText("\n");
