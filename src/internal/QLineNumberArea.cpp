...
QLineNumberArea::QLineNumberArea(QCodeEditor* parent) : QWidget(parent), m_syntaxStyle(nullptr), m_codeEditParent(parent) {
}
...
void QLineNumberArea::paintEvent(QPaintEvent* event) {
    ...
    while (block.isValid() && top <= event->rect().bottom()) {
        if (block.isVisible() && bottom >= event->rect().top()) {
            ...
            painter.drawText(-5, top, sizeHint().width(), m_codeEditParent->fontMetrics().height(), Qt::AlignRight, number);
        }
